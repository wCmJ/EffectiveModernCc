...
- nc
  - 快速构建网络连接，可以以服务器方式运行，也可以以客户端方式运行
  - -i:设置数据包传送的时间间隔
  - -l:以服务器方式运行，监听指定端口，默认以客户端方式运行
  - -k:重复接收并处理某个端口上的所有连接，必须与-l一起使用
  - -p:以客户端运行时，强制使用该端口
  - 

- strace
  - 测试服务器性能的重要工具，跟踪程序运行过程中执行的系统调用和接收到的信号，并将系统调用名、参数、返回值和信号名输出到标准输出或者指定的文件
...
- lsof
  - 列出当前系统打开的文件描述符的工具
  - -i:显示socket文件描述符. lsof -i 4, lsof -i@0.0.0.0:22
...
- netstat
  - 输出本地网卡上的全部连接、路由信息（route）、网卡接口信息（ifconfig）
  - -n:
  - -a:
  - -t:
  - -r:
...
- vmstat
  ...
  - iostat获得磁盘使用情况
  - mpstat获得CPU使用情况
  - 实时输出系统的各种资源的使用情况，比如进程信息、内存使用、cpu使用率及IO使用情况
  - -f:
  - -s:
  - -d:
  - -p:
